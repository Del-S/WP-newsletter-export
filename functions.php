...
function news_export_init() {
    ...
    if(!is_admin()) {
        ...
        $ajax_sub_bool = 0;
        if(!has_filter('newsletter_user_subscribe')) { $ajax_sub_bool = 1; }
        ...
        $ajax_subscription = $ajax_sub_bool;
        ...
        wp_localize_script( 'nle_ajax', 'jquery_data', array('ajax_url' => $ajax_url, 'send_url' => $send_url, 'send_method' => $send_method, 'ajax_subscription' => $ajax_subscription, 'sub_url' => $sub_url, 'ajax_confirmation' => $ajax_confirmation, 'confirm_url' => $confirm_url ) );
